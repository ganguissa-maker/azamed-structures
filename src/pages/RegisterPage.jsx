// src/pages/RegisterPage.jsx — Questions avec boutons radio Oui / Non
...
function SectionHoraires({ form, onChange, theme }) {
  ...
  return (
    <div
      ...
      style={{ borderColor: theme.couleurMed + '40', backgroundColor: theme.couleurClair }}
    >
        ...
        <div className="space-y-3 border-t border-gray-200 pt-3">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {JOURS_SEMAINE.map((j) => {
                const sel = (form.joursOuverture || []).includes(j.key);
                return (
                  <button
                    type="button" key={j.key} onClick={() => toggleJour(j.key)}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                value={form.heureOuverture || '08:00'} onChange={onChange('heureOuverture')}
            ...
            <div>
              ...
              <select
                value={form.heureFermeture || '18:00'} onChange={onChange('heureFermeture')}
                className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none"
              >
                {HEURES.map((h) => <option key={h} value={h}>{h}</option>)}
              </select>
...
// ─── Questions Oui / Non (boutons radio) ──────────────────────
  ...
  return (
    <div className="rounded-xl border border-gray-200 overflow-hidden">
      ...
      <div className="divide-y divide-gray-100">
        {questions.map((q) => {
          // form[q.key] peut être : undefined (pas répondu), true (oui), false (non)
              ...
              <div className="flex gap-3">
                ...
                >
                  <span className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${
                    val === true ? 'border-white' : 'border-gray-300'
                  }`}>
                ...
                >
                  <span className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${
                    val === false ? 'border-white' : 'border-gray-300'
                  }`}>
    ...
    <div>
      <div className="text-center mb-4">
        ...
        <p className="text-gray-500 text-sm mt-1">
          Sélectionnez votre type pour voir les champs correspondants
        </p>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 mb-4 flex items-start gap-2">
        ...
        <p className="text-blue-700 text-xs leading-relaxed">
          ...
          <a
            href={`${PUBLIC_URL}/inscription`} target="_blank" rel="noreferrer"
          ...
          return (
            <button
              ...
              className="flex items-center gap-3 p-4 rounded-xl border-2 border-gray-200
                         hover:shadow-md text-left transition-all group"
            >
              <div
  ...
  return (
    <div>
      ...
      <div
        ...
        style={{ background: theme.couleurClair }}
      >
        <div
          className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl shrink-0"
      ...
      <div className="space-y-4 max-h-[62vh] overflow-y-auto pr-1">
        {/* Nom */}
        <Field
          label="Nom de l'établissement" name="nomLegal" required
        ...
        <div className="border-t border-gray-100 pt-4">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field
                label="Contact de l'accueil" name="telephone" required
                placeholder="+237 6XX XX XX XX"
                value={form.telephone} onChange={onChange('telephone')}
              />
              <Field
                label="WhatsApp de l'établissement" name="whatsapp"
                placeholder="+237 6XX XX XX XX"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <Field
                label="Ville" name="ville" required placeholder="Yaoundé"
                value={form.ville} onChange={onChange('ville')}
              />
              <Field
                label="Quartier" name="quartier" placeholder="Bastos"
                value={form.quartier} onChange={onChange('quartier')}
              />
            </div>
            <Field
              label="Adresse complète" name="adresse"
              placeholder="Ex: Rue Kennedy, face à la poste"
        ...
        <div>
          ...
          <textarea
            value={form.description} onChange={onChange('description')} rows={2}
      ...
      <div className="flex gap-3 mt-5 pt-3 border-t border-gray-100">
        ...
        <button
          type="button" onClick={onNext} disabled={!canContinue}
...
function StepCompte({ typeStructure, form, onChange, onSubmit, onBack, loading }) {
  ...
  questions.forEach((q) => {
    ...
    if (form[q.key] === true && label && !modulesActifs.includes(label)) {
      modulesActifs.push(label);
    }
  });
  ...
  const horairesResume = form.horaire24h7j
    ...
    : (form.joursOuverture?.length
        ? `${form.joursOuverture.map(j => j.charAt(0).toUpperCase() + j.slice(1, 3)).join(', ')} · ${form.heureOuverture || '08:00'} – ${form.heureFermeture || '18:00'}`
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="space-y-4">
        <Field
          label="Adresse email" name="email" type="email" required
          placeholder="votre@email.com"
          value={form.email} onChange={onChange('email')}
        />
        <Field
          label="Mot de passe" name="password" type="password" required
        ...
        <div className="rounded-xl p-4 space-y-2" style={{ backgroundColor: theme.couleurClair }}>
          ...
          {[
            ...
            <div key={l} className="flex items-center gap-2 text-xs text-gray-600">
              <Check size={11} style={{ color: theme.couleur }}/>
              <strong>{l} :</strong> {v}
            </div>
          ))}
          ...
          {modulesActifs.length > 0 && (
            <div className="pt-2 mt-1 border-t border-gray-200">
              <p className="text-xs font-semibold text-gray-500 mb-2">
                Modules activés dans votre tableau de bord :
              </p>
        ...
        <div className="flex items-start gap-3 bg-gray-50 p-3 rounded-xl border border-gray-100">
          <input
            type="checkbox" id="cgu" checked={acceptCGU}
      ...
      <div className="flex gap-3 mt-6">
        ...
        <button
          type="submit"
          disabled={loading || !acceptCGU || !form.email || !form.password}
          className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl
                     text-sm font-bold text-white transition-all disabled:opacity-60"
          style={{ backgroundColor: theme.couleur }}
        >
          {loading
...
export default function RegisterPage() {
  ...
  const [form, setForm]          = useState({
    ...
    // Réponses aux questions — undefined = pas encore répondu
    offresServices:     undefined,
    faitExamens:        undefined,
    dispenseMeds:       undefined,
    collaboreAssurance: undefined,
    email: '', password: '',
  });
  ...
  const handleSelectType = (type) => {
    // Réinitialiser les réponses quand on change de type
    setForm((prev) => ({
      ...prev,
  ...
  // Calcule les modules actifs à sauvegarder
  const computeModules = () => {
    ...
    questions.forEach((q) => {
      ...
      // Si réponse = false, on s'assure que le module n'est pas actif (sauf si déjà dans les défauts)
      if (form[q.key] === false && !def[q.active]) {
        modules[q.active] = false;
      }
    });
    return modules;
  };
